...
// FIX: Remove useApp, import api service directly
...
const SignUpPage: React.FC = () => {
  ...
  // FIX: Removed useApp(), as auth functions are in api.ts
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // FIX: Call api service directly with the correct function name
      const { error } = await api.signUpNewUser({ email, password, username, avatarUrl });
      if (error) {
        setError(error.message);
      } else {
        router.push('/dashboard'); // Success, send to dashboard
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
            <Link href="/" className="inline-flex items-center justify-center gap-3 text-2xl font-extrabold tracking-tight">
