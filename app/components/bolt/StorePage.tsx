            ...
            <div className="p-6 flex flex-col flex-grow">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-4 flex-grow">
                    ...
                    {item.itemType === 'TIMED_EFFECT' && item.durationHours && (
                        <span className="block mt-2 font-semibold text-purple-600 dark:text-purple-300">Lasts for {item.durationHours} hours.</span>
                    )}
                    {/* 🟢 Visual Tag for Cosmetics */}
                    {['NAME_COLOR', 'TITLE', 'BANNER'].includes(item.itemType) && (
                        <span className="block mt-2 text-xs font-bold text-blue-600 dark:text-blue-300 uppercase tracking-wide">Cosmetic Item</span>
                    )}
                </p>
                <div className="flex justify-between items-center mb-4">
                    <p className="text-sm text-slate-500 dark:text-slate-400">Cost:</p>
                    <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{item.cost.toLocaleString()} XP</p>
                </div>
                <button
                    onClick={() => onPurchase(item.id)}
                    disabled={!canAfford || isPurchasing}
                    className="w-full bg-purple-600 text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:bg-purple-700 transition-colors disabled:bg-slate-300 dark:disabled:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-70"
                >
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Spend your XP on powerful items and cosmetics.</p>
                </div>
                <div className="flex gap-6 text-right">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg">
                        <p className="text-sm text-slate-500 dark:text-slate-400">Your XP</p>
                        <p className="text-xl font-bold text-blue-400">{user.xp.toLocaleString()}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg">
                        <p className="text-sm text-slate-500 dark:text-slate-400">Your Freezes</p>
            ...
            {activeItems.length > 0 ? (
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-center p-12 h-64">
