...
const QuestCard: React.FC<QuestCardProps> = ({ quest, progress, onClaim, isClaiming }) => {
    ...
    const renderMiniBadge = () => {
        ...
        return (
            <div className="w-5 h-5 rounded flex items-center justify-center border border-slate-300 dark:border-white/20 bg-slate-100 dark:bg-slate-900" style={{ borderColor: color }}>
    ...
    return (
        <div className={`bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden transition-all duration-300 ${isClaimed ? 'opacity-50' : ''}`}>
            <div className="p-6">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{quest.title}</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-4 min-h-[40px]">{quest.description}</p>
                            ...
                            <div key={index}>
                                <div className="flex justify-between items-center text-sm mb-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`${taskComplete ? 'text-slate-600 dark:text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}>{description}</span>
                                    </div>
                                    <span className="font-mono text-slate-500 dark:text-slate-400">{currentProgress}/{safeTarget}</span>
                                </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-700/50 mt-auto p-4 sm:p-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-4">
                    ...
                    {badgeRewardConfig && (
                        <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded border border-slate-300 dark:border-slate-600">
                            {renderMiniBadge()}
                            <span className="text-xs font-bold text-slate-600 dark:text-slate-300 truncate max-w-[100px]">{badgeRewardName}</span>
                        </div>
                    )}
                </div>
                <button
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">Complete these challenges to earn XP and unique badges.</p>
            </div>
