...
const badgeFromSupabase = (data: any): Badge => ({
  ...
  color: data.color,
  communityId: data.community_id ?? "",
  isActive: data.is_active ?? !data.is_archived, // Support both columns
...
export const api = {
    ...
    // 🟢 FIX: Fetch 'is_active' from DB
    getBadgesConfig: async (): Promise<BadgesConfig> => {
        const communityId = await getCommunityId();
        const { data, error } = await supabase.from('badges').select('name, description, icon, color, is_archived, is_active').eq('community_id', communityId);
        if (error || !data) return {};
        return data.reduce((acc, b) => ({ ...acc, [b.name]: { name: b.name, description: b.description, icon: b.icon, color: b.color, isArchived: b.is_archived, isActive: b.is_active } }), {} as any);
    },
    ...
    // 🟢 FIX: Partial update support for Badges
    ...
    createStoreItem: async (itemData: Omit<StoreItem, 'id' | 'createdAt'>): Promise<boolean> => {
        ...
        const { error } = await supabase.from('store_items').insert({
            community_id: communityId, name: itemData.name, description: itemData.description, cost_xp: itemData.cost, icon: itemData.icon, is_available: itemData.isActive, item_type: itemData.itemType, duration_hours: itemData.durationHours, modifier: itemData.modifier, metadata: itemData.metadata, 
        });
        return !error;
    },
    updateStoreItem: async (itemId: string, itemData: any): Promise<boolean> => {
        const { error } = await supabase.from('store_items').update({
            name: itemData.name, description: itemData.description, cost_xp: itemData.cost, icon: itemData.icon, is_available: itemData.isActive, item_type: itemData.itemType, duration_hours: itemData.durationHours, modifier: itemData.modifier, metadata: itemData.metadata, 
        }).eq('id', itemId);
         return !error;
    },
    ...
    equipCosmetic: async (userId: string, item: StoreItem): Promise<{ success: boolean; message: string }> => {
        ...
        if (item.itemType === 'TITLE') { currentMeta.title = item.metadata?.text; currentMeta.titlePosition = item.metadata?.titlePosition || 'prefix'; }
    ...
    claimQuestReward: async (progressId: number) => {
        try {
            const { data: updatedProgress } = await supabase.from('user_quest_progress').update({ is_claimed: true }).eq('id', progressId).eq('is_completed', true).eq('is_claimed', false).select().single();
        ...
        } catch (error: any) { return { success: false, message: 'An error occurred.' }; }
    },
    getAnalyticsData: async (dateRange: '7d' | '30d'): Promise<AnalyticsData | null> => {
        try {
            ...
            const minimalUser = (p: any): User => ({
                ...
                communityId: p.community_id, streakFreezes: p.streak_freezes, last_action_date: p.last_action_date, badges: [], role: p.role, level: 0
            });
