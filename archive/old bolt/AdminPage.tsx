...
import Link from "next/link"; // Needed for <Link> in your JSX

// ✅ Import the AppContext hook (main data source)
import { useApp } from "@/context/AppContext";
import { useContext } from "react";
import { AppContext } from "@/context/AppContext";

// ✅ Import Reward type from AppContext, not types.ts (to prevent mismatched definitions)
import type { Reward } from "@/types";

// ✅ Other shared types from your types.ts
import type {
  Profile,
  RewardAction,
  ActionType,
...
export default function AdminPage() {
  ...
  const targetUser = allUsers.find((u: User) => u.id === targetUserId);
...
const handleAwardXp = async () => {
  if (targetUser) {
    ...
    showNotification(
      `Awarded ${gained} XP for ${actionType.replace(/_/g, " ")} to ${targetUser.username}.`
    );
  }
};
...
const handleAwardBadgeClick = async () => {
  ...
  const handleRewardSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editRewardAction) {
await handleUpdateReward(editRewardAction, { xp: newActionXp, badge: null } as any);
...
showNotification(`Added new reward worth ${newActionXp} XP.`);
...
const handleDeleteRewardClick = async (actionType: string) => {
  if (window.confirm(`This will archive the reward "${actionType}" if it has been used, or delete it permanently if it has not. Continue?`)) {
    await handleDeleteReward(actionType); // ✅ Only one argument
...
const allBadgesForShowcase: Badge[] = Object.entries(badgesConfig).map(
  ([name, config], index) => ({
    id: `cfg_${index}`,
    communityId: "", // not used in the showcase
    isActive: true,
    name,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-slate-800 p-6 rounded-2xl shadow-lg relative overflow-hidden">
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center text-center p-4 rounded-2xl z-10">
                ...
                <p className="text-sm text-slate-300">
                 ...
                 <Link href="/pricing" className="mt-4 bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-purple-700 transition-colors">
                    View Plans
                </Link>
            </div>
            ...
            {Object.entries(rewardsConfig).map(([key, value]) => (
                <div key={key} className="flex justify-between items-center bg-slate-700/50 p-2 rounded-md">
                    <div>
                        ...
                        <span className="font-bold text-blue-400 ml-4">{(value as Reward).xpGained} XP</span>
                    </div>
                    <div className="flex gap-2">
      ...
      <div className="bg-slate-800 p-6 rounded-2xl shadow-lg">
        ...
        <form onSubmit={handleAddOrEditBadge} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end bg-slate-700/50 p-4 rounded-lg mb-6">
