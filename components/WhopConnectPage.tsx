...
const WhopConnectPage: React.FC = () => {
    ...
    }, [isWhopConnected, navigate]);

    const handleAuthorize = async () => {
        setIsConnecting(true);
        await connectWhop();
        // The API call in connectWhop has a delay, so the loading state feels natural.
        // The useEffect will handle the navigation once isWhopConnected is true.
    };
    ...
    if (!community) {
         return (
            <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white text-center">
                <div>
                    <p className="mb-4">Could not load community data. Please log in again.</p>
                    <button onClick={() => navigate('/')} className="bg-blue-600 text-white font-bold py-2 px-6 rounded-lg">Go to Home</button>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white p-4">
            <div className="max-w-md w-full bg-slate-800 rounded-2xl shadow-lg p-8 border border-slate-700 text-center">
                <div className="flex justify-center items-center gap-4 mb-6">
                    <img src={community.logoUrl} alt={community.name} className="w-16 h-16 rounded-lg object-cover" />
                ...
                <h1 className="text-2xl font-bold text-white mb-2">Connect to Whop</h1>
                <p className="text-slate-400 mb-6">
                    <span className="font-semibold text-white">ApexDM Score</span> is requesting permission to access your <span className="font-semibold text-white">{community.name}</span> Whop store data.
                </p>
